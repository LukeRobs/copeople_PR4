...
export default function EstacaoTable({ estacoes, onEdit, onDelete }) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:`repeat(${cols}, 1fr)`,gap:16,padding:20}}>
      {estacoes.map(e => <EstacaoCard key={e.idEstacao} estacao={e} T={T} onEdit={onEdit} onDelete={onDelete}/>)}
    </div>
  );
}
    ...
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
          <Avatar name={e.nomeEstacao} T={T}/>
          <div style={{minWidth:0}}>
            <p style={{fontSize:15,fontWeight:700,color:T.textMain,margin:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{e.nomeEstacao}</p>
            <p style={{fontSize:11,color:T.textSubtle,margin:"3px 0 0"}}>ID #{e.idEstacao}</p>
          </div>
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6,paddingTop:4,borderTop:`1px solid ${T.divider}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:12,color:T.textSubtle}}>Regional</span>
          <span style={{fontSize:12,color:T.textMuted,fontWeight:500}}>{e.regional?.nome || "—"}</span>
        </div>
      </div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,paddingTop:10,borderTop:`1px solid ${T.divider}`}}>
        <ActionBtn label="Editar" icon={<Pencil size={13}/>} bg={T.btnEditBg} border={T.btnEditBorder} color={T.btnEditText} hoverBg={T.btnEditHover} onClick={()=>onEdit(e)}/>
        <ActionBtn label="Excluir" icon={<Trash2 size={13}/>} bg={T.btnDelBg} border={T.btnDelBorder} color={T.btnDelText} hoverBg={T.btnDelHover} onClick={()=>onDelete(e)}/>
      </div>
    </div>
  );
}
