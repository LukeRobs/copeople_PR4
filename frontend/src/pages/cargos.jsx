...
export default function CargosPage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch(err) { console.error(err); setCargos([]); }
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,color:T.textMain}}>
        ...
        <main style={{padding:"28px 32px",maxWidth:1280,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div>
                <h1 style={{fontSize:22,fontWeight:700,color:T.textMain,margin:0}}>Cargos</h1>
                <p style={{fontSize:13,color:T.textSubtle,margin:"3px 0 0"}}>Gestão de cargos e níveis hierárquicos</p>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              {!loading && (
                <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 14px",borderRadius:999,
                  ...
                  <Briefcase size={13}/>{cargos.length} cargo{cargos.length!==1?"s":""}
                </div>
              )}
          ...
          <div style={{background:T.card,border:`1px solid ${T.cardBorder}`,borderRadius:16,overflow:"hidden"}}>
            {loading ? <LoadingScreen message="Carregando cargos..."/> :
             cargos.length===0 ? (
               <div style={{padding:"60px 20px",textAlign:"center",color:T.emptyText,display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
                 <Briefcase size={40} strokeWidth={1.2} style={{opacity:0.35}}/><p style={{fontSize:14}}>Nenhum cargo encontrado</p>
               </div>
             ) : (
               <CargoTable cargos={cargos}
                 onEdit={c=>{setSelected(c);setModalOpen(true);}}
                 onDelete={async c=>{
                   if(!window.confirm(`Excluir cargo "${c.nomeCargo}"?`))return;
                   await CargosAPI.excluir(c.idCargo); load();
                 }}/>
             )}
          </div>
        </main>
      ...
      {modalOpen && (
        <CargoModal cargo={selected} onClose={()=>setModalOpen(false)}
          onSave={async data=>{
            if(selected) await CargosAPI.atualizar(selected.idCargo,data);
            else await CargosAPI.criar(data);
