...
export default function EstacoesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEstacoes(list);
    } catch(err) { console.error(err); alert("Erro ao carregar estações"); }
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,color:T.textMain}}>
        ...
        <main style={{padding:"28px 32px",maxWidth:1280,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div>
                <h1 style={{fontSize:22,fontWeight:700,color:T.textMain,margin:0}}>Estações</h1>
                <p style={{fontSize:13,color:T.textSubtle,margin:"3px 0 0"}}>Gestão de estações e vínculos com regionais</p>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              {!loading && (
                <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 14px",borderRadius:999,
                  ...
                  <Radio size={13}/>{estacoes.length} {estacoes.length!==1?"estações":"estação"}
                </div>
              )}
          ...
          <div style={{background:T.card,border:`1px solid ${T.cardBorder}`,borderRadius:16,overflow:"hidden"}}>
            {loading ? <LoadingScreen message="Carregando estações..."/> :
             estacoes.length===0 ? (
               <div style={{padding:"60px 20px",textAlign:"center",color:T.emptyText,display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
                 <Radio size={40} strokeWidth={1.2} style={{opacity:0.35}}/><p style={{fontSize:14}}>Nenhuma estação encontrada</p>
               </div>
             ) : (
               <EstacaoTable estacoes={estacoes}
                 onEdit={estacao=>{setSelected(estacao);setModalOpen(true);}}
                 onDelete={async estacao=>{
                   if(!window.confirm(`Deseja excluir a estação "${estacao.nomeEstacao}"?`))return;
                   await EstacoesAPI.excluir(estacao.idEstacao); load();
                 }}/>
             )}
          </div>
        </main>
      ...
      {modalOpen && (
        <EstacaoModal estacao={selected} onClose={()=>setModalOpen(false)}
          onSave={async data=>{
            if(selected) await EstacoesAPI.atualizar(selected.idEstacao,data);
            else await EstacoesAPI.criar(data);
